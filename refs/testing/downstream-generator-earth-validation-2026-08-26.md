...
The longitudinal pressure and regional-desert audit is complete. The fixed latitude term dominates the pressure attribution, but widening it is non-selective; basin centers and thermal pressure are too small to explain the annual hydration gap. The next material slice should prototype a cheap, final-circulation-aligned precipitation refinement on the fixed 128 x 64 grid, with separate evidence for upwind barriers and coastal fetch. It must not amplify the earlier topology-wind fields or introduce named-Earth rules. Require cross-tier improvement in biome F1 and regional desert recovery while preserving permanent-ice semantics, inland gains, humid/dry controls, latitude error, regional ordering, memory, and performance. Keep Earth and expensive performance diagnostics outside routine push CI.
