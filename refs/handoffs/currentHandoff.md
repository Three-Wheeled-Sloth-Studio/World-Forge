...
Basin circulation v10 retains permanent-ice liquid semantics and bounded convergence recycling and now carries a materially stronger north-equatorial countercurrent. Standard/Ultra wetness rank is 0.602/0.645 and Köppen macro-F1 is 0.574/0.586. The ocean audit protects western-boundary intensification (1.39 Standard), equatorial flow/counterflow (0.769), gyre rotation (0.961), land confinement (1.0), and Southern circumpolar continuity (0.912). The evaluator-only seasonal pressure model improves coarse reconstructed macro-F1 by 0.014/0.021/0.029, but both production paths were rejected: the late raster path regressed Standard, while preserving early topology/cohesion output produced only 0.5304 Fast and regressed Standard to 0.5489. All production changes were removed. A native-resolution/project-then-aggregate counterfactual now reproduces that Standard regression (0.5712 versus 0.5735 control) and is the required production acceptance screen. Next, use that screen on another generic ecological signal. Do not feed answer keys into production, add named-region rules, retune annual hydration coefficients, mutate annual wetness, reapply the ice factor, or allocate Ultra-scale seasonal layers.
