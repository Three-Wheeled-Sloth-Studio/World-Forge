...
Basin circulation v9 retains permanent-ice liquid semantics and bounded convergence recycling. Standard/Ultra wetness rank is 0.602/0.645 and false-dry rate is 0.559/0.522. Köppen macro-F1 is 0.529/0.574/0.586 after corrected reference coverage and temperature-adjusted forest moisture demand. The longitudinal audit found that fixed latitude pressure dominates missed-desert attribution, but widening it is non-selective; regional errors have different mechanisms. Final-wind coastal/barrier signals, continental limitation, and a low-cost two-season water-balance proxy were measured with an explicit zero-adjustment control. None produced a cross-tier end-to-end gain worth its regressions, and all production prototypes were removed. Annual hydration coefficient tuning is at a stopping point. Next, strengthen fixed-grid ocean-circulation validation—western-boundary intensification, equatorial currents, subtropical convergence, and circumpolar flow—before changing ocean behavior. Do not feed answer keys into production, add named-region rules, retune latitude belts, reapply the ice factor, or allocate Ultra-scale seasonal layers.
