...
Fixed-scale coastal transport, cool-current stability, permanent-ice liquid semantics, bounded continental convergence recycling, corrected biome coverage, and basin circulation v10 are implemented. Annual hydration tuning is at a stopping point. The ocean audit now protects western-boundary, equatorial, gyre, confinement, and circumpolar structure. An evaluator-only explicit seasonal pressure diagnostic improves coarse reconstructed macro-F1 by 0.014/0.021/0.029 Fast/Standard/Ultra, but both production paths were rejected: the late raster overlay regressed Standard, and preserving early topology/cohesion output improved Fast only to 0.5304 while regressing Standard to 0.5489. All production changes were removed. A native-resolution/project-then-aggregate counterfactual now reproduces the real Standard regression (0.5712 versus 0.5735 control) and must screen future production candidates. Next, evaluate another generic ecological signal through both coarse and native-first paths. Protect annual wetness semantics, permanent ice, inland gains, regional ordering, latitude error, memory, and performance; do not allocate Ultra-scale seasonal arrays.
