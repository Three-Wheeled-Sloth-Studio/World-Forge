...
Native confusion tables localize temperature, generated/reference wetness, hydrology, circulation, and classifier margins without retaining native samples. The optional GLWD evaluator remains manual and answer-key data never enters production. The opt-in `catchment-budget-v1` model now reuses live accumulation, preserves a scale-aware incumbent share, fills the remaining exact wetland budget through a bounded histogram, and survives cohesion/final circulation. It improves GLWD recall to `40.83% / 30.17% / 25.70%` and separation to `7.50 / 10.82 / 13.80` points while Köppen macro-F1 remains within tolerance at `0.5468 / 0.5640 / 0.5932`. The 80-case fictional suite adds no aggregate finding and unsupported wetlands remain zero. Next may promote this model as the production default, keeping `lowland-floodplain-v1` as a reversible/manual comparator.
