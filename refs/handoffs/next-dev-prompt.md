...
Fixed-scale wind-aligned coastal-moisture redistribution is implemented and normalized to conserve its donor contribution. The unconditional coastal bonus is now orientation-aware: full under onshore flow, 85% alongshore, and 50% under fully offshore flow. This improves dry-coast errors across the maintained tiers while keeping coarse orography positive. The residual dry-coast error remains large, and a stronger coefficient calibration was rejected after it reversed Fast orographic ordering. Next, diagnose whether generic cool-current/upwelling influence can be exposed to coastal hydration within the present stage order and what observation evidence is needed before changing production coupling. Protect transport conservation, inland gains, false-wet rate, latitude error, regional guards, memory, and performance.
