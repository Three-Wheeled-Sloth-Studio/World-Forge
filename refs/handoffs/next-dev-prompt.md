...
Fixed-scale coastal transport, cool-current stability, permanent-ice liquid semantics, bounded continental convergence recycling, corrected biome coverage, and basin circulation v10 are implemented. Annual hydration tuning is at a stopping point. The ocean audit now protects western-boundary, equatorial, gyre, confinement, and circumpolar structure. An evaluator-only explicit seasonal pressure diagnostic improves reconstructed macro-F1 by 0.014/0.021/0.029 Fast/Standard/Ultra. A late raster seasonal overlay improved Fast but regressed Standard and was removed. Next, integrate the fixed-grid seasonal signal before topology biome classification/cohesion, then project once. Protect annual wetness semantics, permanent ice, inland gains, regional ordering, latitude error, memory, and performance; do not allocate Ultra-scale seasonal arrays.
