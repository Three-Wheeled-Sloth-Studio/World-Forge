...
Native confusion tables localize temperature, generated/reference wetness, hydrology, circulation, and classifier margins without retaining native samples. Remaining grassland/forest and desert confusion is dominated by upstream wetness error. The optional local GLWD v2 evaluator separately measures wetlands without leaking the answer key into production or CI. Its latest attribution shows riverine floodplains have the strongest observed association, while a flat saturated non-river predicate is unsafe. A follow-up two-ring drainage-margin and cold-flatland proxy is directionally enriched but also rejected: the Ultra union selects `314,851` cells and only `9.53%` are high-coverage reference wetlands. Do not ship another global saturation threshold or hydrology-neighborhood dilation. Next expose the solver's already-computed transient raw accumulation and downstream receivers through an optional manual-validation observer, then screen a true catchment-retention/topographic-wetness proxy. Normal generation must allocate and persist nothing new when that observer is absent.
