...
function catalogEntryForBody(
  ...
  const eligibleByScaffold = body.kind === 'moon' && !isPrimary && body.placeholder;
  return {
    id: body.id,
    label: isPrimary ? project.projectName : bodyLabel(project, body),
    kind: body.kind,
    parentBodyId: body.parentBodyId,
    generationStatus: lifecycleRecord?.status ?? (isPrimary || !body.placeholder ? 'generated' : eligibleByScaffold ? 'ready' : 'placeholder'),
    generationEligible: lifecycleRecord?.eligible ?? eligibleByScaffold,
    generationProfile: lifecycleRecord?.profile ?? (eligibleByScaffold ? 'airless-rocky-body' : null),
