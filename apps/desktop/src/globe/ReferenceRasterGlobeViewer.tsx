...
import {
  createReferenceRasterBodyPresentation,
  referenceRasterGeometryLabel,
  referenceRasterShapeScale,
} from '../presentation/referenceRasterBodyPresentation';
...
export function ReferenceRasterGlobeViewer({
  project,
  orbitalContext,
  target,
  simulationClock,
  ...
  useEffect(() => {
    ...
    host.dataset.globeSurfaceTextureDetail = `${presentation.sourceResolution.width}x${presentation.sourceResolution.height}`;

    const starfield = createStarfield(`${orbitalContext.seed}:${target.bodyId}`);
    scene.add(starfield);
    scene.add(new THREE.AmbientLight(0xa8b4ba, 0.24));
    const sun = new THREE.DirectionalLight(orbitalContext.payload.star.colorHex, 3.35);
    sun.castShadow = true;
  ...
  const scale = target.rasterDetail
    ? referenceRasterShapeScale(target.rasterDetail)
    : { x: 1, y: 1, z: 1 };
  const geometry = target.rasterDetail
    ? referenceRasterGeometryLabel(target.rasterDetail)
    : 'smooth-reference-sphere';
  const albedo = target.rasterDetail?.assets?.find((asset) => asset.role === 'albedo' && asset.resolution);

  return (
    <div
      className="globe-viewer"
      aria-label={`Reference globe for ${target.label}`}
      data-globe-target-body={target.bodyId}
      data-globe-target-mode={target.mode}
      data-globe-surface-material="imported-reference-rgb565"
      data-globe-surface-geometry={geometry}
      data-globe-scale={`${scale.x.toFixed(6)},${scale.y.toFixed(6)},${scale.z.toFixed(6)}`}
      data-globe-source-resolution={albedo?.resolution
        ? `${albedo.resolution.width}x${albedo.resolution.height}`
        : 'unknown'}
    ...
    >
      ...
      <section className="globe-body-target-status" aria-label="Globe target">
        ...
        <small>Imported reference surface</small>
        <button
          type="button"
