...
if (await globe.getAttribute('data-cloud-renderer') !== 'sparse-layered-noise-v3') throw new Error('Sparse layered cloud renderer is not active.');
if (await globe.getAttribute('data-cloud-coverage-profile') !== 'clear-sky-gaps') throw new Error('Clear-sky coverage profile is not active.');
if (await globe.getAttribute('data-weather-shell-offset') !== '0.002') throw new Error('Weather systems are still riding on the old high shell.');
...
console.log(JSON.stringify({ distanceBefore, distanceAfter, renderer: await globe.getAttribute('data-cloud-renderer'), coverageProfile: await globe.getAttribute('data-cloud-coverage-profile'), weatherShellOffset: await globe.getAttribute('data-weather-shell-offset') }, null, 2));
await browser.close();
