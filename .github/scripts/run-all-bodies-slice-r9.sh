...
source = source.replace(
    ...
    "BRANCH='automation/all-system-bodies-20260801-r9'",
    1
)
marker = ")\nPYINTEGRATE\ngit diff --check"
addition = r''')
...
Path('/tmp/run-all-bodies-slice-r9.sh').write_text(source.replace(marker, addition, 1), encoding='utf-8')
PY
bash /tmp/run-all-bodies-slice-r9.sh
