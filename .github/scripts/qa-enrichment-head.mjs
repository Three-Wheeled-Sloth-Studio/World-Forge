...
if (statusText.includes('Â')) throw new Error(`Mojibake remained in orbital status: ${statusText}`);

for (const viewport of [{ width: 1440, height: 900 }, { width: 1920, height: 1080 }]) {
  await page.setViewportSize(viewport);
  await page.waitForTimeout(100);
  const overflow = await page.evaluate(() => ({ y: document.documentElement.scrollHeight - window.innerHeight }));
...
await graph.locator('.graph-toolbar-fields select').first().selectOption('project.system-orbital-context');
...
if (await nodes.count() !== 6 || await completed.count() !== 6) {
  throw new Error(`Expected six completed enrichment nodes; nodes=${await nodes.count()}, complete=${await completed.count()}.`);
}
